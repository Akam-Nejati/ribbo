            ...
            <div class="grid grid-cols-6 gap-14">
                <div class="col-span-2 rounded-xl bg-gray-100 dark:bg-slate-900 p-4">
                    ...
                    <div class="flex flex-col gap-4 mt-4">
                        <span v-for="tag in tags.data" :key="tag.id" class="p-4 rounded-xl bg-blue-400 text-xl">{{tag.attributes.tag}}</span>
                    </div>
                </div>
                <div
                    ...
                    <div class="w-full bg-gray-100 dark:bg-slate-900 rounded-xl flex justify-start gap-4 p-4"
                        v-for="post in posts.data" :key="post.id">
                        <div class="w-[20rem]">
                            <img class="rounded-xl h-full" :src="`http://localhost:1337${post.attributes.thumbnail.data.attributes.url}`" alt="">
                        </div>
                        <div class="w-[calc(100%-20em)] flex flex-col justify-between gap-4">
                            ...
                            <p
                                class="my-6 h-[4.5rem] text-md font-normal text-gray-700 dark:text-gray-400 text-justify line-clamp-[3]">{{ post.attributes.details }}</p>
                        </div>
                    </div>
                </div>
            </div>
